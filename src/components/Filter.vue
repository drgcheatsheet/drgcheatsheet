...
<template>
  <div>
    ...
    <div class="flex gap-4 sm:gap-8 justify-center mt-4 mb-6 mx-6">
      <div v-for="character in characters" :key="character" class="relative h-10 w-10 sm:h-18 sm:w-18">
        <img
          :src="getImageUrl(`icon_character_${character}.png`)"
          :alt="character.toLowerCase()"
          class="absolute z-10 w-10 sm:w-18 cursor-pointer drg-filter-picture"
          :class="!filter[character.toLowerCase()].value && 'grayscale'"
          @click="toggleClass(character.toLowerCase())"
        >
        <div
          class="absolute top--1 left--1 w-12 h-12 sm:h-20 sm:w-20 drg-filter-picture"
